({
    //ft-128
    setDefaultAccountRole: function(component, businessPartnerRole) {
         var action = component.get("c.getDefaultAccountRole");
        action.setParams({"businessPartnerRole": businessPartnerRole});
        action.setCallback(this, function(response) {
            var state = response.getState();
            if(state === "SUCCESS") { 
                var defaulARValue = response.getReturnValue();
                if(defaulARValue != null && defaulARValue != '')
                {
                	var accountRoleCmp = component.find("account_role");
                    var accountRoles = component.get("v.accountRoleMap");
                  	accountRoleCmp.set("v.value",accountRoles[defaulARValue]);
                }
            } else {
                console.log('Problem getting default role value, response state: ' + state);
            }
        });
